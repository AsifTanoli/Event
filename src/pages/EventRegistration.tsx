...
export default function EventRegistration() {
  ...
  const FORM_VALUES_STORAGE_KEY = `event-registration-form-${routeEventId || "default"}`;
  ...
  const saveFormValuesToStorage = (values: FormValues) => {
    try {
      ...
      console.warn("[EventRegistration] Failed to save form values to sessionStorage:", error);
    }
  };
  ...
  const loadFormValuesFromStorage = (): FormValues | null => {
    try {
      ...
      console.warn("[EventRegistration] Failed to load form values from sessionStorage:", error);
    }
    return null;
  };
  ...
  const clearFormValuesFromStorage = () => {
    try {
      ...
      console.warn("[EventRegistration] Failed to clear form values from sessionStorage:", error);
    }
  };
  ...
  useEffect(() => {
    ...
    if (forceRefresh && routeEventId) {
      try {
        ...
        console.warn("[EventRegistration] Failed to clear localStorage cache:", error);
      }
    }
    ...
    const loadSchema = async () => {
      ...
      try {
        ...
        if (routeEventId) {
          try {
            ...
            if (apiSchema && apiSchema.fields && apiSchema.fields.length > 0) {
              ...
              setSchema(apiSchema);
              setIsExpired(apiSchema.isRegistrationClosed ?? false);
              setDataSource("API");
              ...
              if (routeEventId) {
                try {
                  ...
                  console.warn("[EventRegistration] Failed to update localStorage cache:", error);
                }
              }
            ...
            console.error("[EventRegistration] Error loading schema from API:", error);
        ...
        try {
          ...
          // Only log errors for debugging
          console.error("[EventRegistration] Error loading schema from localStorage:", error);
        ...
        // Only log errors for debugging
        console.error("[EventRegistration] Unexpected error loading schema:", error);
        if (isMounted) {
          setSchema({ ...fallback, eventId: idToUse });
          setIsExpired(false);
          setDataSource("fallback");
  ...
  useEffect(() => {
    return () => {
      ...
      if (submitted && routeEventId) {
        try {
          sessionStorage.removeItem(`event-registration-form-${routeEventId}`);
        } catch (error) {
  ...
  const handleUAE_PASSAuth = async () => {
    ...
    const uaePassAuthUrl = `${import.meta.env.VITE_GCAA}/en/Pages/Login.aspx?eventurl=${window.location.href}?attendeeid=`;
    ...
    try {
      ...
      if (result.verified) {
        ...
        if (submissionValues && Object.keys(submissionValues).length > 0) {
          ...
          console.error("[EventRegistration] Form data lost: pendingSubmissionValues, sessionStorage, and formValues all empty");
        }
      } else {
      ...
      <div className="container mx-auto py-12 px-4 sm:px-6 lg:px-8 max-w-xl">
        <div className="bg-white rounded-xl shadow-sm p-8 text-center space-y-6">
          ...
          <div className="pt-4 border-t border-gray-200">
            <p className="text-sm text-amber-600">The registration period for this event has ended.</p>
          </div>
        </div>
      </div>
    );
    ...
    <div className="min-h-screen w-full flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <div className="w-full max-w-3xl relative">
          ...
          <div className="relative">
            ...
            <div className="flex items-start justify-between mb-4 pb-4">
              ...
              <div className="flex-1 text-center px-4">
                <h1 className="text-3xl font-bold text-gray-900 mb-2">{schema?.eventTitle || "Event Registration"}</h1>
                {displayableMethods.length > 0 && !isManualOnlyMethod ? (
                  <p className="text-gray-600">Choose your preferred registration method and complete the form below</p>
                ) : (
                  <p className="text-gray-600">Please complete the registration form below</p>
                )}
              </div>
        ...
        <PageLoader
          ...
          description="Please wait while we prepare your registration form. This will only take a moment."
          showProgress={true}
          progress={progress}
        />
      </div>
