...
import { useAuth, getPostLoginRedirectTarget } from "../presentation/context/AuthContext";
...
export default function Login() {
  ...
  const [returnTo] = useState<string>(() => {
    ...
    if (state?.pathname && state.pathname.startsWith("/")) {
      return `${state.pathname}${state.search || ""}${state.hash || ""}`;
    }
    return getPostLoginRedirectTarget();
  });
